...
/**
 * VYVRE Pricing — English client component (style AURA·OS V6)
 * Same visual identity as FR, EN copy + EUR primary with USD/GBP indicative conversion
 */

import { useState } from 'react';

// ── Stripe Payment Links (LIVE) — same as FR (Stripe billing in EUR) ──
const STRIPE_LINKS = {
...
export default function PricingClientEn({ brandSlug }: { brandSlug: string }) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center mb-10">
          <div className="inline-flex items-center gap-1 p-1 border border-line rounded-full text-xs font-mono tracking-[0.12em] uppercase backdrop-blur">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full transition-colors ${annual ? 'bg-text text-bg' : 'text-text/55 hover:text-text'}`}
            >Annual <span className="text-accent">· -17%</span></button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card
            ...
            priceSuffix=""
            priceConversion=""
            subtitle="30 days · no commitment"
            features={[
              '1,000 scans / month',
              'Web SDK',
              'VYVRE branding',
              'Email support 48h',
              'France-based infrastructure',
            ]}
          ...
          <Card
            ...
            priceSuffix="/month"
            priceConversion={annual ? '~$269 · ~£215' : '~$320 · ~£260'}
            subtitle={annual ? '€2,990 / year · 2 months free' : '5,000 scans / month'}
            features={[
              '5,000 scans / month',
              '€0.02 per extra scan',
          ...
          <Card
            ...
            priceSuffix="/month"
            priceConversion={annual ? '~$448 · ~£358' : '~$540 · ~£440'}
            subtitle={annual ? '€4,990 / year · 2 months free' : '15,000 scans / month'}
            features={[
              '15,000 scans / month',
              '€0.015 per extra scan',
          ...
          <Card
            ...
            priceSuffix="/month"
            priceConversion={annual ? 'starts at · custom contract' : 'starts at · no commitment'}
            subtitle={annual ? 'custom annual contract' : 'pay monthly · cancel anytime'}
            features={[
              '25,000 scans / month',
              '€0.01 per extra scan',
              'Unlimited retail network',
              'Native mobile app',
              'SLA 99.99% · 24/7 Director',
            ]}
            ctaLabel="Contact us"
        ...
        <div className="mt-10 text-center text-xs text-text/45 font-mono tracking-[0.18em] uppercase">
          Public pricing · VAT not included · Billed in EUR · Cancel anytime
        </div>
        <div className="mt-2 text-center text-[10px] text-text/35 font-mono tracking-[0.15em]">
          USD / GBP shown as indicative conversion (rate refreshed weekly)
        </div>
      </div>
    </section>
  );
...
function Card({
  tier, price, priceSuffix, priceConversion, subtitle, features, ctaLabel, ctaUrl, recommended,
}: {
  tier: string;
  price: string;
  priceSuffix: string;
  ...
  return (
    <div style={recommended ? CARD_RECOMMENDED : CARD_STYLE}>
      ...
      >
        {recommended ? `Plan: ${tier} · Recommended` : `Plan: ${tier}`}
      </span>
